...
inp_rate = sq_rate + 25 * trial_rst$size15_p050
inp_rate[is.na(inp_rate)] = sq_rate
plot(inp_rate)

# coords <- coordinates(trial_rst)
# meig 	  <- meigen_f(coords=coords, model = 'gau', enum = 600)
# saveRDS(meig, 'data/data_meigen.rds')
# 
...
for(rate_rng in rate_rngs){
  for(opt_pct in opt_pcts){
    ...
    for(tname in tnames){
      ...
      s_name = paste0('data/data_trial_',tname, '_rng_', rate_rng, '_opt_', opt_pct, '_2.rds')
      saveRDS(df, s_name)
...
#          yield_pred = b0_pred + b1_pred * n_rate + b2_pred * n_rate**2,
